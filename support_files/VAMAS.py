...
def parseVAMAS(datafile):
    ...
    if experiment_mode in ["MAP", "MAPDP"]:
        ...
        header["number_of_discrete_x_coordinates_available_in_full_map"] = int(
            readline().strip()
        )
        header["number_of_discrete_y_coordinates_available_in_full_map"] = int(
            readline().strip()
        )
    ...
    for block_idx in range(number_of_blocks):
        ...
        if (
            block_idx == 0
            or number_of_entries_in_parameter_inclusion_or_exclusion_list == 0
        ):
        ...
        if 7 in params_available:
            ...
            if number_of_hours_in_advance_of_greenwich_mean_time < 0:
                GMT = " GMT {}".format(
                    number_of_hours_in_advance_of_greenwich_mean_time
                )
            else:
                GMT = " GMT +{}".format(
                    number_of_hours_in_advance_of_greenwich_mean_time
        ...
        if 13 in params_available:
            if experiment_mode in ["MAPDP", "MAPSVDP", "SDP", "SVDP"] or technique in [
                "FABMS",
                "FABMS energy spec",
                "ISS",
                "SIMS",
                ...
                block["number_of_atoms_in_sputtering_ion_or_atom_particle"] = int(
                    readline().strip()
                )
                block["sputtering_ion_or_atom_charge_sign_and_number"] = int(
                    readline().strip()
                )
        ...
        try:
            if 14 in params_available:
                block["analysis_source_characteristic_energy"] = float(
                    readline().strip()
                )
        except ValueError:
            pass
        ...
        # 17
        if 17 in params_available and experiment_mode in [
            "MAP",
            "MAPDP",
            "MAPSV",
            "MAPSVDP",
        ...
        if 19 in params_available:
            block["analysis_source_polar_angle_of_incidence"] = float(
                readline().strip()
            )
        ...
        if 25 in params_available:
            block["analyser_work_function_or_acceptance_energy_of_atom_or_ion"] = float(
                readline().strip()
            )
        ...
        # 37
        if (
            37 in params_available
            and technique in ["AES DIFF", "AES DIR", "EDX", "ELS", "UPS", "XPS", "XRF"]
            and experiment_mode in ["MAPDP", "MAPSVDP", "SDP", "SDPSV"]
        ):
            ...
            block["sputtering_source_polar_angle_of_incidence"] = float(
                readline().strip()
            )
        ...
        number_of_data_points_in_each_variable = (
            number_data_points_in_block // number_of_variables
        )
        ...
        for i in range(number_of_variables):
            ...
            variable_ranges.append(
                "{}: [{}, {}]".format(
                    variables_in_columns[i],
                    minimum_ordinate_value,
